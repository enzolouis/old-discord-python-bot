...
class Blacklist(commands.Cog):
  def __init__(self, bot):
    self.bot = bot
    self.owner = (418154142175854613, 327074335238127616)
      ...
      if ctx.invoked_subcommand is None:
        embed = CtxEmbed(ctx, title="Blacklist module", color=0x51ff00,
          ...
          **But why ?** The bot's owner look for bad comportement and had the bobby to te blacklist, \
          a lot of reason are possible, like MP ad, troll, ... And **you** can report these users.\n\n```fix\nAll commands :```""")
        embed.add_field(name="`blacklist verif (@user)`", value="verify all users blacklisted in the server (or only one user with the `verif @user`)", inline=False)
        embed.add_field(name="`blacklist report [@user] [reason_and_link]`", value="Report an user with evidence (link)", inline=False)
  ...
  async def report(self, ctx, user:discord.User, *, reason):
      embed = discord.Embed(title=f"Report from the {ctx.message.guild} ({ctx.message.guild.id}) by {ctx.author} ({ctx.author.id})")
      embed.add_field(name="User report", value=f"{user} ({user.id})", inline=False)
      embed.add_field(name="Reason", value=reason)
      for x in self.owner:
          owner = self.bot.get_user(x)
          await owner.send(embed=embed)

      await ctx.send("> :white_check_mark: **Report sent to creators !**")
      await ctx.send(embed=embed)
  ...
  async def verif(self, ctx, member:discord.Member=None):
        ...
        if member is None:
            ...
            try:
              ...
              await ctx.send("Error") # changer en un message


        else:
            embed = discord.Embed( color=0x0000cf)
            for elt in result:
                mem_id = elt[0]
                evidence = elt[1]
                if member.id == mem_id:
